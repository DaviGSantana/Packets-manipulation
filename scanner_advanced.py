...
connected_devices.append({"ip": element[1].psrc, "mac": element[1].hwsrc, "hostname": None, "vendor_id":
None})
...
ip_addresses_chunks = [ip_addresses[i:i+NUM_IPS_PER_CHUNK] for i in range(0, len(ip_addresses),
NUM_IPS_PER_CHUNK)]
...
try:
connected_devices["mac"] = connected_devices.apply(lambda x: get_mac(x["ip"]) if x["mac"] is None else
x["mac"], axis=1)
except ValueError:
...
connected_devices,
pd.DataFrame(passive_devices, columns=["ip", "mac", "hostname", "vendor_id"]) # convert the list to a
dataframe
])
# remove duplicate ip addresses with least info
connected_devices = connected_devices.sort_values(["mac", "hostname", "vendor_id"],
ascending=False).drop_duplicates("ip", keep="first")
...
parser.add_argument("-a", "--arp", help="ARP scanner interval in seconds, default is 60 seconds", type=int,
default=60)
parser.add_argument("-u", "--udp", help="UDP scanner interval in seconds, default is 60 seconds", type=int,
default=60)
parser.add_argument("-p", "--icmp", help="ICMP scanner interval in seconds, default is 60 seconds", type=int,
default=60)
parser.add_argument("-i", "--interval", help="Interval in seconds to print the connected devices, default is 5
